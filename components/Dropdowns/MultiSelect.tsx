...
//   const { label, selectData, onChange, require, values } = props;
...
export default function MultiSelect(props: any) {
  ...
  const renderedValues = (
    <div className="flex flex-wrap">
      {personName.length > 1 ? (
        <p key={personName[0]} className="text-sm">
          {selectData.find((item: any) => item.id === personName[0])?.name},{" "}
          {"    +" + (personName.length - 1) + ""}
        </p>
      ) : (
        personName.map((value: any) => (
          <p key={value} className="text-sm">
            {selectData.find((item: any) => item.id === value)?.name},
          </p>
        ))
  ...
  return (
    <div className="w-[180px] m-2 mb-0">
      ...
      <FormControl sx={{ m: 1, margin: 0 }}>
        ...
        >
          {selectData.map((item: any, index: any) => {
            // console.log("---id", values && values.includes(item.id));
            return (
              <MenuItem
                key={index}
                value={item.id ? item.id : item}
              ...
              >
                <Checkbox
                  sx={{ padding: "0 8px" }}
                  checked={
                    (values && values.includes(item.id)) ||
                    personName.indexOf(item.id ? item.id : item) > -1
                  }
                />
                <ListItemText
                  primary={item.name ? item.name : item}
